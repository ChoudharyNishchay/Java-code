...
INSERT INTO departments(department_id,department_name,location_id) VALUES (12,'FakeDept',1700);
...
INSERT INTO employees(employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) VALUES (182,'Sarah','Bell','sarah.bell@sqltutorial.org','650.501.1876','1996-02-04',17,4000.00,123,5);
...
INSERT INTO employees(employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) VALUES (207,'amey','Gietz','amey.gietz@sqltutorial.org','512.123.8181','2021-06-07',1,88300.00,205,null);
...
select employees.first_name, employees.last_name, departments.department_id, departments.department_name
	from employees
    join departments on departments.department_id = employees.department_id
    where departments.department_id = 8 or departments.department_id = 4;
    

##-- 5.) Write a query in SQL to display those employees who contain a letter x to their first name 
...
select employees.first_name, employees.last_name, employees.salary 
	from employees 
    where employees.salary <  (select salary from employees where employee_id = 182);
    
    
##-- 8.) Write a query in SQL to display the first name of all employees including the first name of their manager. 
select  e2.first_name, e1.first_name as "manager"
	from employees e1 
...
select  e1.first_name as "manager", e2.first_name 
	from employees e1 
    cross join employees e2 
    where e1.employee_id = ifnull(e2.manager_id, e1.employee_id) ;-- or e2.manager_id is null 
    -- where ifnull(e1.employee_id, e2.manager_id)  = e2.manager_id;
    
    
    

##-- 12.) Write a query in SQL to display the first name, last name, and department number for those employees 
-- who works in the same department as the employee who holds the last name as Taylor.
select employees.first_name, employees.last_name, departments.department_id, departments.department_name
	from employees
    left join departments on departments.department_id = employees.department_id
    where departments.department_id = (select department_id from employees where last_name = "Taylor");
...
select jobs.job_title, departments.department_name, concat(first_name," ", last_name ) as "full name"
	from employees
    join departments  on departments.department_id = employees.department_id
...
select jobs.job_title, concat(first_name," ", last_name ) as "full name", (jobs.max_salary - employees.salary) as "diff in salary"
	from employees
    join jobs on jobs.job_id = employees.job_id;
    
    -- "-" value in output because of manually added slaray which is way above max salary
...
select distinct departments.department_name, (round((min_salary+max_salary) /2)) as "average salary"
	from employees
    join departments  on departments.department_id = employees.department_id
	join jobs on jobs.job_id = employees.job_id;
...
-- job title and the salary differences to their own job for those employees who is working in the department ID 8. 

select concat(employees.first_name," ", employees.last_name ) as "full name", jobs.job_title, (employees.salary - (select round(avg(salary))  from employees where department_id = 8)) as "diff"
    from employees
    join jobs on jobs.job_id = employees.job_id;
...
select departments.department_name, concat(employees.first_name," ", employees.last_name ) as "full name"
	from employees
    join departments  on departments.department_id = employees.department_id
    join jobs on jobs.job_id = employees.job_id
...
select distinct jobs.job_title, round((min_salary + max_salary) / 2) as "average salary"
	from employees
    join jobs on jobs.job_id = employees.job_id;
...
select departments.department_name ,countries.country_name, locations.city , count(departments.department_name) as "no of emp"
	from employees
	join departments  on departments.department_id = employees.department_id
    join locations on departments.location_id = locations.location_id
    join countries on locations.country_id = countries.country_id
    group by departments.department_name, countries.country_name, locations.city
	having count(departments.department_name) >= 2;
    

 

##-- 22.) Write a query in SQL to display the department name, full name (first and last name) of manager, and their city.
select departments.department_name, concat(employees.first_name," ", employees.last_name ) as "full name", locations.city
	from employees
    join departments  on departments.department_id = employees.department_id
    join jobs on jobs.job_id = employees.job_id
    join locations on departments.location_id = locations.location_id
...
select employees.employee_id, jobs.job_title, (curdate() - employees.hire_date) as "number of days worked"
	from employees
    join jobs on jobs.job_id = employees.job_id 
...
select concat(employees.first_name," ", employees.last_name ) as "full name", employees.salary
	from employees
    join departments  on departments.department_id = employees.department_id
    join locations on departments.location_id = locations.location_id
...
select * -- concat(employees.first_name," ", employees.last_name ) as "full name", jobs.job_title
	from employees
    join departments  on departments.department_id = employees.department_id
    join jobs on jobs.job_id = employees.job_id;
    
    -- NO COMMISSON IN TABLE
    
    
    
##-- 26.) Write a query in SQL to display the department name and number of employees in each of the department.

select departments.department_name, count(departments.department_name) as "number of employees"
		from employees
...
select concat(employees.first_name," ", employees.last_name ) as "full name", countries.country_id, countries.country_name
    from employees
	join departments  on departments.department_id = employees.department_id
    join locations on departments.location_id = locations.location_id
    join countries on locations.country_id = countries.country_id
